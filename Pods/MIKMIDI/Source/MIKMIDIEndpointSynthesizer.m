...
- (void)handleMIDIMessages:(NSArray *)commands
{
	for (MIKMIDICommand *command in commands) {
		...
		if (err) NSLog(@"Unable to send MIDI command to synthesizer %@: %i", command, (int)err);
	}
}
...
- (BOOL)setupAUGraph
{
	...
	if ((err = NewAUGraph(&graph))) {
		NSLog(@"Unable to create AU graph: %i", (int)err);
		return NO;
	}
	...
	if ((err = AUGraphAddNode(graph, &outputcd, &outputNode))) {
		NSLog(@"Unable to add ouptput node to graph: %i", (int)err);
		return NO;
	}
	...
	if ((err = AUGraphAddNode(graph, &instrumentcd, &instrumentNode))) {
		NSLog(@"Unable to add instrument node to AU graph: %i", (int)err);
		return NO;
	}
	
	if ((err = AUGraphOpen(graph))) {
		NSLog(@"Unable to open AU graph: %i", (int)err);
		return NO;
	}
	
	AudioUnit instrumentUnit;
	if ((err = AUGraphNodeInfo(graph, instrumentNode, NULL, &instrumentUnit))) {
		NSLog(@"Unable to get instrument AU unit: %i", (int)err);
		return NO;
	}
	
	if ((err = AUGraphConnectNodeInput(graph, instrumentNode, 0, outputNode, 0))) {
		NSLog(@"Unable to connect instrument to output: %i", (int)err);
		return NO;
	}
	
	if ((err = AUGraphInitialize(graph))) {
		NSLog(@"Unable to initialize AU graph: %i", (int)err);
		return NO;
	}
	
	if ((err = AUGraphStart(graph))) {
		NSLog(@"Unable to start AU graph: %i", (int)err);
		return NO;
	}
...
- (BOOL)selectInstrument:(MIKMIDIEndpointSynthesizerInstrument *)instrument;
{
	...
	for (UInt8 channel = 0; channel < 16; channel++) {
		...
		if (err) {
			NSLog(@"MusicDeviceMIDIEvent() (MSB Bank Select) failed with error %d in %s.", (int)err, __PRETTY_FUNCTION__);
			return NO;
		}
		
		err = MusicDeviceMIDIEvent(instrumentUnit, bankSelectStatus, 0x20, bankSelectLSB, 0);
		if (err) {
			NSLog(@"MusicDeviceMIDIEvent() (LSB Bank Select) failed with error %d in %s.", (int)err, __PRETTY_FUNCTION__);
			return NO;
		}
		
		err = MusicDeviceMIDIEvent(instrumentUnit, programChangeStatus, programChange, 0, 0);
		if (err) {
			NSLog(@"MusicDeviceMIDIEvent() (Program Change) failed with error %d in %s.", (int)err, __PRETTY_FUNCTION__);
			return NO;
		}
	}
...
+ (NSArray *)availableInstruments
{
	...
	dispatch_once(&onceToken, ^{
		...
		if (err) {
			NSLog(@"AudioUnitGetProperty() (Instrument Count) failed with error %d in %s.", (int)err, __PRETTY_FUNCTION__);
			return;
		}
		...
		for (UInt32 i = 0; i < instrumentCount; i++) {
			...
			if (err) {
				NSLog(@"AudioUnitGetProperty() (Instrument Number) failed with error %d in %s.", (int)err, __PRETTY_FUNCTION__);
				continue;
			}
...
+ (instancetype)instrumentWithID:(MusicDeviceInstrumentID)instrumentID;
{
	...
	if (err) {
		NSLog(@"AudioUnitGetProperty() failed with error %d in %s.", (int)err, __PRETTY_FUNCTION__);
		return nil;
	}
